...
% pw image image buffer and image display window
Resource.RcvBuffer(2).datatype = 'int16';
Resource.RcvBuffer(2).rowsPerFrame = rowsPerFrameSA;
Resource.RcvBuffer(2).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(2).numFrames = imgBuffer;
...
Resource.ImageBuffer(2).numFrames = 2;
...
Resource.RcvBuffer(3).rowsPerFrame = rowsPerFrameSA; % number of samples of RF data per channel, default sample rate for A/D 
% is 4x the center frequency of the probe
Resource.RcvBuffer(3).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(3).numFrames = ssaFrames;
...
Receive = repmat(struct('Apod', ones(1,numEl), ...
                        ...
                        'callMediaFunc', 1),1,nr*Resource.RcvBuffer(1).numFrames+Resource.RcvBuffer(2).numFrames+Resource.RcvBuffer(3).numFrames); % call media function [edit]
...
for i = 1:Resource.RcvBuffer(3).numFrames
    k = i+nr*Resource.RcvBuffer(1).numFrames+Resource.RcvBuffer(2).numFrames;
    Receive(k).startDepth = SFormat(2).startDepth;
    Receive(k).endDepth = SFormat(2).startDepth + wlsPer128*ceil(maxAcqLengthPW/wlsPer128);
    Receive(k).framenum = i;      
    Receive(k).bufnum = 3; 
...
% set specific ReconInfo attributes for bmode
...
for i = 1:Resource.RcvBuffer(2).numFrames
    ...
    Event(n).tx = 0;         
    Event(n).rcv = 0;        
    Event(n).recon = 0;      % image reconstruction
    Event(n).process = 0;    % image processing using process(1)
    Event(n).seqControl = 9;
    n = n+1;
end
...
%     Event(n).tx = nr+1;
%     Event(n).rcv = i+nr*Resource.RcvBuffer(1).numFrames+Resource.RcvBuffer(2).numFrames; 
...
% Event(n).seqControl = nsc;
%     SeqControl(nsc).command = 'jump';
%     SeqControl(nsc).argument = save_start;
%     nsc = nsc+1;
% n = n+1;
% 
% %*********************** angled planewave SSA acquisition *****************
% angledSSA_acq = n;
% Event(n).info = 'Switch to SA acquire';
% Event(n).tx = 0;         
% Event(n).rcv = 0;       
...
% angledSSA_loop = n;
% j=1;
% sequenceAngles = zeros(1,Resource.RcvBuffer(3).numFrames);
% for i = 1:Resource.RcvBuffer(3).numFrames
%     Event(n).info = 'Send trigger out.';
%     Event(n).tx = 0;
%     Event(n).rcv = 0; 
...
%     n = n+1;
%     
%     sequenceAngles(i) = pwAngles(j); % save angles of scan
%     j = j+1;
%     if j > na; j = 1; end
% end
%     
...
UI(10).Control = {'UserB3','Style','VsPushButton','Tag','pwImage','Label','Test PW'};
UI(10).Callback = text2cell('%CB#10');
...
%CB#10
pwToggleCallback(hObject,eventdata)
return
%CB#10